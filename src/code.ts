...
const nodeToObject = (node, filterText) => {
    ...
    const filteredProperties = Object.entries(obj)
      .filter(
        (entry) =>
          ...
          String(entry[1]).toLowerCase().includes(filterText.toLowerCase())
      )
      .map((entry) => entry[0]);
    const newObj =
      filteredProperties.length > 0 ? { id: obj.id, name: obj.name } : {};
  ...
  if (node.children)
    obj.children = node.children.map((child) =>
      nodeToObject(child, filterText)
    );
  if (node.masterComponent)
    obj.masterComponent = nodeToObject(node.masterComponent, filterText);
  return obj;
};
...
const inspectNodes = (node) => {
  ...
  if (
    JSON.stringify(oldSelection) !== JSON.stringify(nodes) ||
...
const inspectComponents = (components) => {
  dispatch(
    'updateComponentJson',
    nodeToObject({ children: selectedComponents }, componentFilterText).children
  );
};
...
const inspectStyles = (styles) => {
  dispatch(
    'updateStyleJson',
    nodeToObject({ children: selectedStyles }, styleFilterText).children
  );
};
...
handleEvent('selectNode', (nodeIds) => {
  ...
  if (getPageNode(nodes[0]) !== figma.currentPage)
    figma.currentPage = getPageNode(nodes[0]);
...
handleEvent('inspectComponents', (componentKeys) => {
  Promise.all(
    componentKeys
      .filter((key) => key !== '')
      .map((key) =>
        figma.importComponentByKeyAsync(key.trim().replace(/['"]+/g, ''))
      )
  ).then((components) => {
...
handleEvent('inspectStylesById', (styleIds) => {
  const styles = styleIds
    .filter((id) => id !== '')
    .map((id) => figma.getStyleById(id.trim().replace(/['"]+/g, '')));
...
handleEvent('inspectStylesByKey', (styleKeys) => {
  Promise.all(
    styleKeys
      .filter((key) => key !== '')
      .map((key) =>
        figma.importStyleByKeyAsync(key.trim().replace(/['"]+/g, ''))
      )
  ).then((styles) => {
    selectedStyles = styles;
    inspectStyles(selectedStyles);
